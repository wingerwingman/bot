...
def get_solana_half_hourly_data(start_date, end_date):
    """
    Get Solana (SOL) half-hourly price data from the CoinGecko API.
    ...
    Returns:
        - DataFrame containing Solana (SOL) half-hourly price data
    """
    ...
    while current_timestamp < end_timestamp:
        ...
        url = f"https://api.coingecko.com/api/v3/coins/solana/market_chart/range?vs_currency=usd&from={current_timestamp}&to={next_timestamp}"
...
solana_half_hourly_data = get_solana_half_hourly_data(start_date, end_date)
if solana_half_hourly_data is not None:
    print(solana_half_hourly_data.head())

    # Save the data to a CSV file
    filename = 'solana_half_hourly_historical_data_2_days.csv'
    solana_half_hourly_data.to_csv(filename, index=False, mode='a', header=False)
